...
### get data from coinmetrics.io

start_date_unix <- as.numeric(as.POSIXct("2009-01-03 00:00:00")) # date Bitcoin netwrk launched

end_date_unix <- as.numeric(as.POSIXct("2019-05-21 00:00:00"))
...
btc_data <- mutate(btc_data, Category = ifelse(`price (usd)` >= 7500,'Over $7.5k','Under $7.5k'))

### first line chart

ggplot(data=btc_data, aes(x=date, y=`price (usd)`,color = Category,group=1)) +
  geom_line() +
...
ggplot(x, aes(x="", y=Frequency, fill=Category))+
  ...
  geom_text(aes(label = paste0(format(Frequency,big.mark = ','),"\n days\n (",round(Percent*100), "%)")), position = position_stack(vjust = 0.5), size = 5, fontface = 'bold') + 
  labs(title = "# of Days BTC Opened Above Vs. Below $7,500\n Since 2013-04-28") +
...
nums <- seq(1,520,7) # makes the dataset weekly. use seq(1,520,30) for monthly buy-ins
btc_data_2 <- btc_data_2[nums,]
...
for (i in 1:nrow(btc_data_2)) {
  ...
  btc_data_2$btc_bought[i] <- 10 / btc_data_2$`price (usd)`[i]
  
  btc_data_2$cumulative_market_value[i] <- sum(btc_data_2$btc_bought) * btc_data_2$`price (usd)`[i]
...
ggplot(btc_data_2,aes(x=date, y=value,color = type)) +
  geom_line()+
  labs(title =  "Performance of a Weekly $10 Investment in BTC\n Since BTC's Peak on 2017-12-27",caption = "(Based on data from coinmetrics.io)",color='Line Meaning') +
...
for (i in 2:nrow(btc_data_3)) {
  ...
  btc_data_3$percent_change[i] <- (btc_data_3$`price (usd)`[i]-btc_data_3$`price (usd)`[i-1])/btc_data_3$`price (usd)`[i-1]
